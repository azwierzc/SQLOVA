...
def construct_hyper_param(parser):
    ...
    parser.add_argument("--accumulate_gradients", default=1, type=int,
                        help="The number of accumulation of backpropagation to effectively increase the batch size.")
    ...
    parser.add_argument("--max_seq_length",
                        default=240, type=int,  # Set based on maximum length of input tokens.
...
def get_bert(BERT_PT_PATH, bert_type, do_lower_case, no_pretraining):
    ...
    model_bert = BertModel.from_pretrained("dkleczek/bert-base-polish-uncased-v1", output_hidden_states=True)
...
def test(data_loader, data_table, model, model_bert, bert_config, tokenizer,
    ...
    for iB, t in enumerate(data_loader):
        ...
        if cnt < st_pos:
            continue
        # Get fields
        nlu, nlu_t, sql_i, sql_i, sql_t, tb, hs_t, hds = get_fields(t, data_table, no_hs_t=True, no_sql_t=True)
        ...
        for b, pr_sql_i1 in enumerate(pr_sql_i):
            ...
            results1["table_id"] = tb[b]["id"]
            results1["nlu"] = nlu[b]
            results1["human_readable"] = pr_sql_q[b]
            results.append(results1)
...
if __name__ == '__main__':
    ...
    ## 2. Paths
    path_h = './data'  # '/home/wonseok'
    path_wikisql = './data'  # os.path.join(path_h, 'data', 'wikisql_tok')
    ...
    if not args.trained:
        ...
        path_model_bert = './data/model_bert_best.pt'
        path_model = './data/model_best.pt'
    ...
    if args.do_train:
        ...
        for epoch in range(args.tepoch):
            ...
            if acc_lx_t > acc_lx_t_best:
                ...
                torch.save(state, os.path.join('.', 'data/model_best.pt'))

                state = {'model_bert': model_bert.state_dict()}
                torch.save(state, os.path.join('.', 'data/model_bert_best.pt'))
