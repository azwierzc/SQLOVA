...
def annotate(sentence, lower=True):
    ...
    if nlp is None:
        ...
        nlp = stanza.Pipeline(lang='pl', processors='tokenize, lemma')
    ...
    for s in nlp(sentence).sentences:
        for t in s.tokens:
            words.append(t.text)
            gloss.append(t.text)
            #after.append(t.after)
...
def get_best_match(query, corpus):
    ...
    return difflib.get_close_matches(query, ngrams_text, n=2, cutoff=0)
...
def check_wv_tok_in_nlu_tok(wv_tok1, nlu_t1):
    ...
    """
    g_wvi1_corenlp = []
    nlu_t1_nlp = nlp(" ".join(nlu_t1))
    lemmas = [word.lemma for sentences in nlu_t1_nlp.sentences for tok in sentences.tokens for word in tok.words]
    nlu_t1_low = [tok.lower() for tok in nlu_t1]
    for i_wn, wv_tok11 in enumerate(wv_tok1):
        indexes = []
        wv_tok11_low = [tok.lower() for tok in wv_tok11]
        wv_tok11_low = [wv_tok11_low[0], wv_tok11_low[-1]]
        for wv_tok111 in wv_tok11_low:
            word = difflib.get_close_matches(wv_tok111, lemmas, n=1)
            indexes.append(lemmas.index(word[0]))

        #results = find_sub_list(wv_tok11_low, nlu_t1_low)
        #st_idx, ed_idx = results[0]

        g_wvi1_corenlp.append(sorted(indexes))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dout', default='data/', help='output directory')
    ...
    for split in args.split.split(','):
        fsplit = os.path.join(args.din, split) + '_pl.jsonl'
        ftable = os.path.join(args.din, split) + '.tables.pl.jsonl'
        fout = os.path.join(args.dout, split) + '_pl_tok.jsonl'

        print('annotating {}'.format(fsplit))
        with open(fsplit, "r",  encoding="utf-8") as fs, open(ftable, "r",  encoding="utf-8") as ft, open(fout, 'wt', encoding="utf-8") as fo:
